...
namespace Zn.Core.Sockets
{
    public sealed class AsyncTcpSocketSession
    {
        ...
        public AsyncTcpSocketSession(TcpClient tcpClient,
                AsyncTcpSocketServerConfiguration configuration, ISegmentBufferManager bufferManager,
                IAsyncTcpSocketServerEventDispatcher dispatcher, AsyncTcpSocketServer server)
        {
            if (tcpClient == null)
        ...
        internal async Task Start()
        {
            ...
            try
            {
                ...
                _log.Exception(string.Format("Session started for [{0}] on [{1}] in dispatcher [{2}] with session count [{3}].",
                        this.RemoteEndPoint,
        ...
        private void SetSocketOptions()
        {
            ...
            if (_configuration.KeepAlive)
            {
                _tcpClient.Client.SetSocketOption(SocketOptionLevel.Socket,
                    SocketOptionName.KeepAlive, (int)_configuration.KeepAliveInterval.TotalMilliseconds);
            }
        ...
        private async Task Close(bool shallNotifyUserSide)
        {
            ...
            {
                _log.Exception(string.Format("Session closed for [{0}] on [{1}] in dispatcher [{2}] with session count [{3}].",
                                this.RemoteEndPoint,
