    ...
    : this(IPAddress.Any, listenedPort, dispatcher, configuration)
        ...
        public void Listen()
        {
            ...
            try
            {
                ...
                Task.Factory.StartNew(async () => await Accept(), TaskCreationOptions.LongRunning).Forget();
            }
        ...
        private async Task Accept()
        {
            try
            {
                while (IsListening)
                {
                    ...
                    Task.Factory.StartNew(async () => await Process(tcpClient), TaskCreationOptions.None).Forget();
                }
            }
            catch (Exception ex) when (!ShouldThrow(ex)) { }
            catch (Exception ex)
        ...
        private async Task Process(TcpClient acceptedTcpClient)
        {
            ...
            {
                _log.Exception($"New session [{session}]");
                try
                {
                    await session.Start();
                }
                ...
                finally
                {
                    ...
                    if (_sessions.TryRemove(session.SessionKey, out throwAway))
                        _log.Exception($"Close session [{throwAway}]");
                }
            }
        }
        ...
        public async Task SendToAsync(string sessionKey, byte[] data, int offset, int count)
        {
            ...
            if (_sessions.TryGetValue(sessionKey, out sessionFound))
            {
                ...
                _log.Exception($"Cannot find session [{sessionKey}]");
        }
        ...
        public async Task SendToAsync(AsyncTcpSocketSession session, byte[] data, int offset, int count)
        {
            ...
            {
                _log.Exception($"Cannot find session [{session}].");
            }
        }
