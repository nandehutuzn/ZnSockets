    ...
    public class TcpSocketServer
    {
        ...
        private void HandleTcpClientAccepted(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                try
                {
                    _sessions.AddOrUpdate(session.SessionKey, session, (n, o) => o);
                    session.Start();
        ...
        private bool ShouldThrow(Exception ex)  //永远返回false？
        {
        ...
        public void SendTo(string sessionKey, byte[] data, int offset, int count)
        {
            ...
            if (_sessions.TryGetValue(sessionKey, out session))
                ...
                _log.Exception(string.Format("Cannot find session [{0}].", sessionKey));
        }
