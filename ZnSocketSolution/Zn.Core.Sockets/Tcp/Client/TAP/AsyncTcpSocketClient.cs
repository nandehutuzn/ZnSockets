    ...
    public class AsyncTcpSocketClient
    {
        ...
        public async Task Connect()
        {
            ...
            try
            {
                ...
                _log.Exception(string.Format("Connected to server [{0}] with dispatcher [{1}] on [{2}].",
                    this.RemoteEndPoint,
                ...
                {
                    Task.Factory.StartNew(async () => await Process(), TaskCreationOptions.None).Forget();
                }
                else
                {
            ...
            var validateRemoteCertificate = new RemoteCertificateValidationCallback(
                        ...
                        =>
                        {
                            ...
                            if (_configuration.SslPolicyErrorsBypassed)
                                ...
                                _log.Exception(string.Format("Error occurred when validating remote certificate: [{0}], [{1}].",
        ...
        private async Task Close(bool shallNotifySide)
        {
            ...
            if (shallNotifySide)
            {
                _log.Exception(string.Format("Disconnected from server [{0}] with dispatcher [{1}] on [{2}].",
                 this.RemoteEndPoint,
                 _dispatcher.GetType().Name,
                 DateTime.UtcNow.ToString(@"yyyy-MM-dd HH:mm:ss.fffffff")));
                try
        ...
        {
            _log.Exception(string.Format("Client [{0}] error occurred in user side [{1}].", this, ex.Message));
